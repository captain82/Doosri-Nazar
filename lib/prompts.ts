...
export const LENS_POOL: string[] = [
  ...
  `MONEY SHAPE: thinks in daily ₹10 to 50; income arrives daily or only post-harvest; a fee is a "how many days of food" calculation, not a price; needs the full total upfront, pay-later, or cash.`,
  ...
  `PHONE NOT PRIVATE / NOT THEIRS: shared or monitored phone; cannot take a private call; account or number belongs to a husband/son; only stolen 1 to 2 minute windows to use it.`,
  ...
  `TIME & RHYTHM: only gets 1 to 2 minute windows and is constantly interrupted (loses progress); or money and free time follow farming/festival cycles that clash with fixed weekday slots.`,
...
Style: never use em-dashes, en-dashes, or dashes as punctuation anywhere in your output; use commas, colons, or periods instead.
...
// write everything.
export const INFER_SYSTEM = `You are shown the screens of a mobile app flow, in the order a user meets them. From these screens, infer three things for a product-testing tool:
...
- description: 2-3 plain-English sentences tracing what this flow lets a user do across the screens, the steps in order, and any payment or verification you can see. Concrete, no fluff.

Give your best guess even when unsure. Never use em-dashes, en-dashes, or dashes as punctuation; use commas, colons, or periods instead. Return ONLY the structured output. No preamble.`;
...
Style: never use em-dashes, en-dashes, or dashes as punctuation; use commas, colons, or periods instead.`;
